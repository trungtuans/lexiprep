...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function modifyPracticeButtons() {
    ...
    const currentPageElement = document.querySelector('.proqyz__course-page.proqyz__course-cp');
    if (currentPageElement) {
      currentPage = parseInt(currentPageElement.getAttribute('data-page-index')) || 1;
    }
    
    console.log('Current page detected:', currentPage); // Debug log
    
    // Get all practice buttons in exam-detail
    const practiceButtons = document.querySelectorAll('.exam-detail .practice-item__btn');
    ...
    const skipCount = (currentPage === 1) ? 4 : 0;
    
    console.log('Skipping first', skipCount, 'buttons'); // Debug log
    
    // Determine pricing URL based on current page URL
    const pricingUrl = window.location.href.includes('/vi/') ? '/vi/bang-gia/' : '/pricing';
  ...
  const paginationContainer = document.querySelector('.proqyz__course-pagination');
  ...
  const observer = new MutationObserver(function(mutations) {
    mutations.forEach(function(mutation) {
      if (mutation.target.classList.contains('proqyz__course-page')) {
        ...
        if (mutation.oldValue && mutation.oldValue.includes('proqyz__page-loadiing') && 
