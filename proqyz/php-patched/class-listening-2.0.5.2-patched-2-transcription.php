...
class Listening extends Questions{
    public $post_id                 = null;
    ...
    public function __construct($params = []) {
        ...
        if($quiz_settings !== null){
            ...
            $this->is_loc = $qus_loc;
            $audio_settings = isset($quiz_settings->audio)? (object) $quiz_settings->audio : null;
            $this->audio_obj = $audio_settings;
        }
    ...
    public function get_sections_html() {
        if (count($this->sections) > 0) {
            foreach ($this->sections as $skey => $section) {
                ...
                $passage_content = (string) $section->content;
     ...
     * changes @1.2.1.2
     */
    public function get_listening_styles(){
        $site_url = site_url();
        $audio_controls = true;

        if($this->audio_obj !== null){
            $audio_controls = isset($this->audio_obj->controls)? $this->audio_obj->controls : true;
        }
        ob_start(); ?>
        <?php if( $this->result ){ ?>
        <style>
        ...
        <style>
            ...
            .listening-test-result .answer .b-r {
                font-weight: bold;
                color: red;
            }

            .listening-test-result .sl-item.explanation {
                margin: 0px 0px 5px 0px !important;
            }

            .listening-test-result .sl-item.explanation .sl-control {
                top: 0;
            }
            .listening-test-result .sl-item .sl-control {
                display: inline-block;
            ...
            .listening-test-result .sl-item .sl-control a {
                display: inline-block;
                line-height: 25px;
                min-height: 25px;
                padding: 0px 10px;
            ...
            .listening-test-result .sl-control a span.icon-explain {
                background-image: url(<?php echo $site_url; ?>/wp-content/plugins/spacetree/libs/proqyz/includes/templates/public/quiz/dist/img/ielts/listening/icon_explain_blue.png);
            }

            .listening-test-result .sl-item .sl-control a span {
            ...
            a span.icon-listen {
                background-image: url(<?php echo $site_url; ?>/wp-content/plugins/spacetree/libs/proqyz/includes/templates/public/quiz/dist/img/ielts/listening/icon_listen1.png);
                width: 15px;
                height: 15px;
            }
    ...
    public function header(){
        if( $this->result ) {
            ...
            <head>
                ...
                <?php echo $this->get_listening_styles(); ?>
                <script>
    ...
    public function footer(){
        ob_start(); ?>
            <script id="session-details" type="application/json">
                <?php 
                    echo json_encode( (object) [
                        ...
                        // added: chatbot transcript payload
                        "transcript"    => $this->get_transcript_data(),
                    ]); 
                ?>
            </script>
            ...
            <!-- expose transcript to JS + sessionStorage -->
            <script>
            (function(){
                try {
    ...
    // Lightweight transcript extractor for chatbot context
    private function clean_transcript_content( $content ) {
        // Transform placeholders to plain text:
        // - {[text][Q1]} -> text
        $content = preg_replace('/\{\s*\[([^\]]+)\]\s*\[[^\]]*\]\s*\}/s', '$1', (string) $content);
        // - |(text)(Q1)| -> text
        $content = preg_replace('/\|\s*\(([^)]+)\)\s*\([^|]*\)\s*\|/s', '$1', (string) $content);
        // Fallbacks: strip remaining braces/pipes but keep inner content
        $content = preg_replace('/\{([^{}]+)\}/s', '$1', (string) $content);
        $content = preg_replace('/\|([^|]+)\|/s', '$1', (string) $content);

        // Keep simple HTML where useful (p, br) and normalize whitespace
        $allowed = '<p><br>';
        ...
        // Plain text version
        $text = trim(preg_replace('/\s+/', ' ', html_entity_decode(strip_tags($content))));

        return (object)[ 'html' => $html, 'text' => $text ];
    }

    public function get_transcript_data() {
        $out = [];
        if (!empty($this->sections)) {
            foreach ($this->sections as $idx => $section) {
                ...
                $clean   = $this->clean_transcript_content($content);
                $out[] = [
                    'section' => $idx + 1,
                    'title'   => $title,
                    'html'    => $clean->html,
    ...
    public function get_layout(){
        ...
        if($this->audio_obj !== null){
            $audio_url  = isset($this->audio_obj->src)? (string) $this->audio_obj->src : '';
            /// replacer to cdn
            $audio_url = str_replace(ST_OLD_CDN_ENDPOINT, ST_CDN_ENDPOINT, $audio_url);
            // $audio_controls = isset($this->audio_obj->controls)? $this->audio_obj->controls : true;
        }
        ...
        if( $this->result ) {
            ...
            if(is_user_logged_in()){
                $class_body = $this->resume? '-resume' : '-start disabled-controls';
                $class_body .= ' user-logged-in ';
                if( $this->resume ){
                    $audio_time = isset($this->user_data->quiz->audio_time)? $this->user_data->quiz->audio_time : 0;
                }
            } else {
            ...
            <body class="listening-test show-palette take-test-page -practice-mode user-logged-in has-glyphicons <?php echo $class_body; ?>">
                <div class="dialog-off-canvas-main-canvas js-attempt-only-listening">
                    
                    <?php if( is_user_logged_in() && !$this->resume ){ ?>
                    <div class="take-test__click-play -practice" style="display: block;">
                        <div class="take-test__play-btn"> Loading ...</div>
                        <div class="take-test__play-btn take-test__play-btn_click" style="" id="js-click-play"> 
                            Click<span>here</span> to start the test
                        </div>
                    </div>
                    <?php } ?>
                    ...
                    <header class="realtest-header ">
                        <span class="realtest-header__logo practice-item__icon -listening d-none-sm-550px"></span>
                        <?php if(isset($this->quiz->title)) { ?>
                    ...
                    <div class="take-test__player-wrap">
                        <div class="take-test__player-container <?php echo $audio_controls? '' : '--no-controls'; ?>">
                            <audio 
                                src="<?php echo $audio_url; ?>" 
                                class="" 
                                id="take-test__player"
                                data-audio-time="<?php echo $audio_time;  ?>"
                                data-audio-controls="<?php echo $audio_controls; ?>"
                            ></audio>
                        </div>
                    </div>
                            ...
                            <div class="region region-content">
                                <article role="article">
                                    <div class="take-test__board highlighter-context" id="highlighter-contents">
                                        <div class="take-test__questions-wrap">
    ...
    public function get_result_layout(){
        ...
        ob_start(); ?>
            ...
            <html <?php language_attributes(); ?>>
            <?php echo $this->header(); ?>
            <body style="background-color:#f5f5f5f7;" class="listening-result listening-test listening-test-result show-palette take-test-page -practice-mode <?php echo $class_body; ?>">
                
                <div class="dialog-off-canvas-main-canvas">

                    <header class="realtest-header ">
                        <span class="realtest-header__logo practice-item__icon -listening "></span>
                        <?php if(isset($this->quiz->title)) { ?>
                        ...
                        <div class="realtest-header__btn-group">
                            <a href="/" class="realtest-header__bt-submit -icon-chevron-left -listening--btn" style="background: #f5f5f5;color: #296239;">
                                Home
                            </a>
                            <a href="<?php echo $this->attempt_url; ?>" class="realtest-header__bt-submit -listening--btn">
                                Retake
                            </a>
                    ...
                    <div class="modal-dialog modal-fullscreen" style="display:contents;">
                        <div class="modal-content p-0 m-0" style="display:contents;">
                            <div class="dialog-off-canvas-main-canvas js-attempt-only-reading" style="background:white;">
                                ...
                                <div class="take-test__player-wrap">
                                    <div class="take-test__player-container">
                                        <audio src="<?php echo $audio_url; ?>" data-audio-time="0" class="" id="take-test__player" data-audio-controls="1"></audio>
                                    </div>
                                </div>
